...
const getConnectionPoint = (component: Component, side: Side, scale: number) => {
  const sx = component.position.x * scale
  const sy = component.position.y * scale
  const points: Record<Side, { x: number; y: number }> = {
    top: { x: sx + PREVIEW_COMPONENT_WIDTH / 2, y: sy },
    right: { x: sx + PREVIEW_COMPONENT_WIDTH, y: sy + PREVIEW_COMPONENT_HEIGHT / 2 },
    bottom: { x: sx + PREVIEW_COMPONENT_WIDTH / 2, y: sy + PREVIEW_COMPONENT_HEIGHT },
    left: { x: sx, y: sy + PREVIEW_COMPONENT_HEIGHT / 2 },
  }
  return points[side]
}
...
const getBestConnectionSides = (fromComponent: Component, toComponent: Component) => {
  if (fromComponent.id === toComponent.id) {
    return { fromSide: 'top' as Side, toSide: 'top' as Side }
  }
  ...
  if (Math.abs(dx) > Math.abs(dy)) {
    return { fromSide: (dx > 0 ? 'right' : 'left') as Side, toSide: (dx > 0 ? 'left' : 'right') as Side }
  }
  return { fromSide: (dy > 0 ? 'bottom' : 'top') as Side, toSide: (dy > 0 ? 'top' : 'bottom') as Side }
}

const generateSimplePath = (fromPoint: { x: number; y: number }, toPoint: { x: number; y: number }, isSelfConnection = false) => {
  if (isSelfConnection) {
    const loopSize = 15
    return `M ${fromPoint.x} ${fromPoint.y} C ${fromPoint.x + loopSize} ${fromPoint.y - loopSize} ${fromPoint.x + loopSize} ${fromPoint.y + loopSize} ${fromPoint.x} ${fromPoint.y}`
  }
...
export const PreviewCanvas: React.FC<PreviewCanvasProps> = ({
  ...
  const { scale, offsetX, offsetY } = useMemo(() => {
    ...
    components.forEach(comp => {
      ...
      maxX = Math.max(maxX, comp.position.x + PREVIEW_COMPONENT_WIDTH + 25)
      maxY = Math.max(maxY, comp.position.y + PREVIEW_COMPONENT_HEIGHT + 25)
    })
    ...
    const aspectFactor = aspectRatio > 3 ? 0.6 : aspectRatio > 2 ? 0.7 : aspectRatio > 1.5 ? 0.8 : 1
    ...
    const ox = (width - scaledWidth) / 2 - minX * s
    const oy = (height - scaledHeight) / 2 - minY * s
  ...
  const renderedConnections = useMemo(() => {
    return connections.map((conn, index) => {
      ...
      const fromPoint = getConnectionPoint(fromComponent, fromSide, scale)
      const toPoint = isSelfConnection ? fromPoint : getConnectionPoint(toComponent, toSide, scale)
      fromPoint.x += offsetX; fromPoint.y += offsetY
  ...
  const simplifiedConnections = useMemo(() => {
    ...
    return connections.map((conn, index) => {
      ...
      const toComponent = components.find(c => c.id === conn.to)
      if (!fromComponent || !toComponent) return null
      if (fromComponent.id === toComponent.id) return null
  ...
  return (
    <div
      className={`relative bg-gradient-to-br from-slate-50 to-slate-100 border-2 border-blue-300 rounded-lg overflow-hidden ${className}`}
      ...
      {components.length === 0 && (
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="text-center text-slate-500">
            <div className="text-2xl mb-2">&bull;</div>
